            ...
            <div className="w-[80%] font-bold text-white mx-auto flex justify-between items-center">
                ...
                <div className="md:w-10 md:h-10 w-8 h-8 flex items-center justify-center rounded-full transition-150" >
                    <div className="relative" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
                        ...
                        <div className={`absolute transition-300 z-50 top-10 right-0 rounded-xl text-black bg-white  overflow-hidden ${hover ? "max-h-52" : "max-h-0"}`}>
                            <div className="px-4 py-2 border rounded-xl">
                                <p onClick={() => {
                                    navigate("/dashboard/profile")
                                }} className="cursor-pointer">Profile</p>
                                <hr className="my-2" />
                                <p className="cursor-pointer text-danger" onClick={() => {
                                    handleLogout();
                                    navigate("/");
                                }}>Logout</p>
                            </div>
